...
import torch
from torch._dynamo._trace_wrapped_higher_order_op import Tensor
...
@instantiate_parametrized_tests
class TestCuTeDSLGroupedGemm(TestCase):
    def _get_inputs(
        self,
        group_size: int,
        M_hint: int,
        ...
        M_sizes = (
            torch.randint(1, (M_hint // 64) + 1, (group_size,), dtype=torch.int) * 64
        )
    ...
    def test_grouped_gemm_basic(self, group_size: int, M_hint: int, K: int, N: int):
        ...
        torch.testing.assert_close(c_eager, c_compiled, rtol=1e-2, atol=0.05)

    @parametrize("layout_A", ("contiguous", "offset", "padded"))
    ...
    def test_grouped_gemm_assorted_layouts(
        ...
        A = torch.randn(sum_M, K, device=device, dtype=dtype)
...
if __name__ == "__main__":
    if HAS_CUTLASS and is_datacenter_blackwell_arch():
        run_tests()
