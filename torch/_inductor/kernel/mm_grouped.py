...
from ..utils import (
    ...
    use_cutedsl_blackwell_template,
    use_triton_template,
)
...
try:
    from torch._inductor.kernel.templates.cutedsl_mm_grouped_configs import (
        get_default_groupgemm_configs,
    )
    from torch._inductor.kernel.vendored_templates.cutedsl_grouped_gemm import (  # noqa: F401
        GroupedGemmKernel,
    )
...
cutedsl_grouped_mm_template = CuteDSLTemplate(
    ...
    source=load_template("cutedsl_mm_grouped", _TEMPLATE_DIR),
)


def can_use_blackwell_cutedsl_grouped_mm(
    layout: Layout,
    a_is_2d: bool,
    b_is_2d: bool,
    ...
    return (
        CUTE_AVAILABLE
        and use_cutedsl_blackwell_template(layout)
        and a_is_2d
        and not b_is_2d
...
def _tuned_grouped_mm_common(
    ...
    if can_use_blackwell_cutedsl_grouped_mm(
        layout, a_is_2d, b_is_2d, offs, bias, scale_result
    ):
        for config in get_default_groupgemm_configs():
            kwargs = dict(
